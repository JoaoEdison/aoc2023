...
#define MAX_CONSEC 10
#define MIND 4

void shortest()
{
	struct iteration curr, *ptri;
	char jumpo, jumpac, jumpmin;
	...
	while ((ptri = pop_minheap(queue))) {
		...
		if (curr.d == 0){
			jumpo = 2;
			if (curr.ac == MAX_CONSEC)
				jumpac = 1;
			else if (curr.ac < MIND)
				jumpmin = 1;
		}else if (curr.d == 1){
			jumpo = 1;
			if (curr.ac == MAX_CONSEC)
		...
		if (curr.x+1 < len && jumpo != 1 && jumpac != 1 && (!jumpmin || jumpmin == 1))
			add_edge(curr.x+1, curr.y, 0, curr.d==0? curr.ac+1 : 1, curr.dist);
		if (curr.x-1 >= 0 && jumpo!= 2 && jumpac != 2 && (!jumpmin || jumpmin == 2))
			add_edge(curr.x-1, curr.y, 1, curr.d==1? curr.ac+1 : 1, curr.dist);
		if (curr.y-1 >= 0 && jumpo != 3 && jumpac != 3 && (!jumpmin || jumpmin == 3))
			add_edge(curr.x, curr.y-1, 2, curr.d==2? curr.ac+1 : 1, curr.dist);
		if (curr.y+1 < wid && jumpo != 4 && jumpac != 4 && (!jumpmin || jumpmin == 4))
...
main()
{
	...
	for (i=0;i<TABLE_SIZE;i++)
		for (next=table[i].head; next; next = next->next)
			if (next->data.x == len-1 && next->data.y == wid-1 && min > next->data.dist && next->data.ac >= MIND)
				min = next->data.dist;
	printf("%d\n", min);
}
