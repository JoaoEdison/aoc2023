...
#include <stdlib.h>
#include <string.h>
#include "/home/joao/programacao/c/estruturas-base/linkedlist.c"
...
struct list table[TABLE_SIZE] = { 0 };
	...
	while ((c = getchar())!='\n'){
		if (c == '{')
			...
			*p = '\0';
			...
			if (first) {
				*p = '\0';
				append_list(&table[hashstr(new->names[0])], new);
				first=0;
				p = bufferin;
				*p++ = c;
			} else
		...
		for (next=table[i].head;next;next=next->next) {
			...
			for (k=0;k<curr->numcat;k++)
				if (curr->names[k+1][0] == 'A')
					...
					curr->next[k] = find_list(&table[hashstr(mock.names[0])], &mock, compare_workflows);
				}
		}
}
...
main()
{
	...
	in = find_list(&table[hashstr("in")], &mock, compare_workflows);
	sum=0;
